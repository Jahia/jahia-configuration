...
public class ManifestElement {
	...
	public static ManifestElement[] parseHeader(String header, String value) throws Exception {
		...
		parseloop: while (true) {
			...
			if (next == null)
				throw new Exception("Invalid value [" + value + "] for header [" + header + "]");
			...
			while (c == '=' || c == ':') {
				...
				try {
					...
					throw new Exception("Invalid value [" + value + "] for header [" + header + "]", e);
				}
				c = tokenizer.getChar();
				if (c == ';') /* more */{
	 ...
	 */
	public static Map<String, String> parseBundleManifest(InputStream manifest, Map<String, String> headers) throws IOException, Exception {
		if (headers == null)
		...
		try {
			...
			while (true) {
				...
				if (line.charAt(0) == ' ') /* continuation */
				{
					...
					{
						throw new Exception("Invalid space detected in line " + line);
					}
				...
				if (colon == -1) /* no colon */
				{
					throw new Exception("Missing colon at line " + line);
				}
