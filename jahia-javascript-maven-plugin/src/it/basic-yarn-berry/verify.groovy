...
// Verify that 'yarn clean' was executed
assert logContent.contains("Clean command executed!") : "Yarn clean command was not executed or failed"
println "✓ Clean executed successfully"

// Verify that 'yarn package' was executed
def packageTgz = new File(basedir, "dist/package.tgz")
assert packageTgz.exists() : "Package archive (dist/package.tgz) was not created"
println "✓ Package archive created"

// Verify that the package.tgz was attached
assert logContent.contains('basic-yarn-berry/dist/package.tgz attached successfully') : "package.tgz did not get attached to the Maven project"
println "✓ Package archive attached"


// Verify that 'yarn verify' was executed
assert logContent.contains("Verification passed!") : "Yarn verify command was not executed or failed"
